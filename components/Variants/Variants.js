  ...
  return (
    <div className="flex flex-col gap-3 max-lg:w-full">
      {variantOptions.map((item) => {
        return (
          <div className="flex items-center gap-4 max-md:flex-col max-md:items-start max-sm:rounded">
            <label
              htmlFor={item.id}
              className="font-light max-lg:text-left max-sm:rounded max-sm:pl-2 max-sm:uppercase md:text-[20px] md:font-extralight"
            >
              {item.attribute.name}
            </label>
            <div
              key={item.id}
              id={item.id}
              className="flex-wrap items-center gap-5 py-2 max-md:w-full max-sm:flex max-sm:p-2 max-sm:text-right sm:flex md:w-[60%]"
            >
              {(item.values ?? [])?.map((value) => {
                  ...
                  >
                    <Image
                      onClick={(e) => {
                        ...
                        if (variant_product) {
                          updateProductVariant(variant_product);
                          handleURLChange(variant_product?.link?.link_path);

                          product_slug = variant_product?.link?.link_path;
                        } else {
                          updateProductVariant(null);
                        }
                      }}
                      alt={`${value.name} ${value.selected}`}
                      selected={value.selected}
                      style={{ display: value.display }}
                      className={`${
                        display === "show"
                          ? `block max-sm:text-right`
                          : `hidden`
                      } cursor-pointer rounded-full border-2 border-transparent hover:border-[#a16d57] ${
                        selected?.find((x) => x.value_key === value.key)
                          ? `!border-[#a16d57]`
                          : ``
                      }`}
