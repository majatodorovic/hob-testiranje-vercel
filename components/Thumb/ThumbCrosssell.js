...
const ThumbCrosssell = ({
  ...
  const products = (data ?? []).map((product, index) => {
    ...
    return (
      <SwiperSlide key={product?.basic_data?.id}>
        <div
          className="item md:ease w-full rounded-2xl p-2 px-[6px] pt-2 md:transition-all lg:px-[16px]"
          onMouseEnter={() =>
        ...
        >
              ...
              {product?.image?.map((image, index) => (
                <SwiperSlide key={index} className={`!overflow-hidden`}>
                  {image && (
                    ...
                    >
                      <Image
                        ...
                        className={`z-[50] h-full w-full rounded-[30px] object-cover opacity-100 group-hover:scale-105 md:transition-all md:duration-200`}
                      />
                    </Link>
                  )}
                  <div
                    ...
                    className={`favorites absolute bottom-4 left-4 z-[20] cursor-pointer rounded-full p-1`}
                  >
                    {!isInWishlist ? (
                      <>
                        <Image
                  ...
                  <div
                    ...
                    className="absolute bottom-4 right-4 z-[100] cursor-pointer rounded-full bg-[#9f7262] px-[1rem] pb-4 pt-3 md:px-4"
                  >
                    <Image
                      src={cart}
                      alt="cart"
          ...
          <div className="relative z-[50] mt-[0.813rem] px-3 pt-[12px]">
            ...
            >
              <p className="clamp2 mb-2 pl-2 text-left text-[20px] leading-[22px] max-md:leading-[20px] max-md:text-[0.85]">
  ...
  if (slider) {
    ...
    const products = data?.map((product, index) => {
      ...
      return (
        <div
          className="item hoveredColor md:ease relative col-span-1 rounded-2xl px-2 py-2 md:transition-all 2xl:px-4"
          onMouseEnter={() => {
            setNavigationEnabled({
              enabled: true,
              id: product?.basic_data?.id_product,
